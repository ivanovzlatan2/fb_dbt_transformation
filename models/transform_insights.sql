...
basic_report_actions as (select date, ad_id,_time_loaded,campaign_id, adset_id, ad_name, adset_name,account_name, campaign_name, cpc, cpm, ctr, frequency, impressions,inline_link_clicks,reach,objective,spend,
...
from base
group by base.ad_id, base.campaign_id,base._time_loaded,base.adset_id,base.date, ad_name, adset_name,account_name,campaign_name, cpc, cpm, ctr, frequency, impressions,inline_link_clicks,reach,objective,spend
...
select basic_report_actions.date as date, basic_report_actions.ad_id, _time_loaded, campaign_name,adset_name, ad_name,account_name,  adset_id, campaign_id, cpc, cpm, ctr, frequency, impressions, inline_link_clicks, reach,objective, spend, view_content, add_to_cart,add_to_cart_value,purchase,purchase_1dc,purchase_7dc,purchase_28dc,purchase_value_1dc,purchase_value_7dc,purchase_value_28dc,landing_page_view, purchase_value, comment,lead, messages, messages_7d, initiate_checkout, complete_registration from basic_report_actions
...
select basic_report_final.date as date, basic_report_final.ad_id, _time_loaded, basic_report_final.campaign_name,basic_report_final.adset_name, ad_name,basic_report_final.account_name,  adset_id, basic_report_final.campaign_id, cpc, cpm, ctr, frequency, impressions, inline_link_clicks, reach,objective, spend, view_content, add_to_cart,add_to_cart_value,purchase,purchase_1dc,purchase_7dc,purchase_28dc,purchase_value_1dc,purchase_value_7dc,purchase_value_28dc,landing_page_view, purchase_value, comment,lead, messages, messages_7d, initiate_checkout, complete_registration, current_campaign_name.campaign_name as current_campaign_name from basic_report_final
...
select * from (
  select
    ...
    row_number() over(partition by ad_id, date order by _time_loaded desc) as rn
    from merge_ad_names
    ) t
    where t.rn = 1